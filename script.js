document.addEventListener('DOMContentLoaded', () => {
  ...
  // 12. Spark Minda Interactive Plant Map
  const mapElement = document.getElementById('spark-minda-map');
  if (mapElement && typeof L !== 'undefined') {
    const chakanCoords = [18.7604, 73.8617];
    const map = L.map('spark-minda-map', {
      center: chakanCoords,
      zoom: 11,
    ...
    const locations = [
      {
        name: 'Spark Minda Die Casting Division (Internship Site)',
        lat: 18.7604,
        lng: 73.8617,
        popup: '<b>Spark Minda Die Casting Division</b><br>MIDC Chakan Industrial Area, Pune, MH<br><i>GDC & LPDC Die Casting Plant</i>',
        featured: true
      },
      {
        name: 'Spark Minda Security Systems (Chakan Phase-III)',
        lat: 18.7730,
        lng: 73.8520,
        popup: '<b>Spark Minda Security Systems</b><br>Chakan Phase-III MIDC, Pune',
        featured: false
      },
      {
        name: 'Spark Minda Corporate HQ',
        lat: 28.5355,
        lng: 77.3910,
        popup: '<b>Spark Minda Corporate HQ</b><br>Noida / Greater Noida, UP',
        featured: false
      },
      {
        name: 'Spark Minda Chennai Plant',
        lat: 12.9801,
        lng: 80.0583,
        popup: '<b>Spark Minda Manufacturing Facility</b><br>Chennai Industrial Belt, TN',
        featured: false
      }
    ];
    ...
    const selectedLocNote = document.querySelector('.map-footer-note span:last-child');
    ...
    mapPills.forEach(pill => {
      pill.addEventListener('click', () => {
        ...
        const zoom = parseInt(pill.getAttribute('data-zoom'), 10);
        const name = pill.getAttribute('data-name');

        map.flyTo([lat, lng], zoom, { duration: 1.2 });

        if (selectedLocNote) {
          selectedLocNote.innerHTML = `<strong>Selected Location:</strong> ${name}`;
        }
      });
    });
  }
